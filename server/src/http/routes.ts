...
export function registerRoutes(app: FastifyInstance, users: UserStore, rooms: RoomService): void {
  ...
  app.post('/api/voice/token', async (_req, reply) => {
    void reply.status(501)
    return { code: 'VOICE_NOT_CONFIGURED', message: '实时语音尚未配置（M2）' }
  })
}
